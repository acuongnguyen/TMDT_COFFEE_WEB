...
const Login = (props) => {
	...
	const handleLogin = async (e) => {
		e.preventDefault();
		if (!email) {
			setEmailError("Tên đăng nhập không được để trống!")
			setPwdError('')
		...
		try {
			const respond = await axios.post(`${backendAPI}/api/app/auth/login`, {
				userName: email,
				password: password,
				device_token: "xxx111xxx"
			})
			localStorage.setItem('tokenU', respond.data.data.token);
