...
const Order = (props) => {
    const { id, uuid, orderItems, totalOrderValue, stateId, stateName, createdAt, setIsCreateFeedbackModalOpen, setIsUpdateFeedbackModalOpen, setProductVariantIdForFeedBack } = props;
    ...
    return (
        <Link className='order border-radius' href={`/get-order/${id}`}>
            ...
            <div className="order-header border-radius d-flex align-items-center justify-content-between">
                <div>
                    <p className='order-title fw-bold'>#{uuid}</p>
