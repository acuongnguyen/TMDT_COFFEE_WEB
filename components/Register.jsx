...
const Register = (props) => {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            const respond = await axios.post(`${backendAPI}/api/app/auth/register`, {
                userName: userName,
                password: password,
                name: fullName,
                phone: phoneNumber,
            ...
            const response = await axios.post(`${backendAPI}/api/app/auth/login`, {
                userName: userName,
                password: password,
                device_token: "xxx111xxx"
            })
            dispatch(customerLoginOrRegister(response.data.data))
            props.toClose();
        } catch (error) {
            let errorMessage = '';

            if (error.response.data.error.phone) {
                errorMessage = error.response.data.error.phone;
